...
void main ()
{
	mat4 tempMat = mvp * toWorld[gl_InstanceID];
	gl_Position = tempMat * vec4(Position.xyz, 1.0f);

	vec4 normal = normalize(toWorld[gl_InstanceID] * vec4(Normal.xyz, 0.0f));
	vec4 pos = toWorld[gl_InstanceID] * vec4(Position.xyz, 1.0f);
    ...
    if (color[gl_InstanceID].w == 0) // Special case for Sun, so that it is lit from its top
    ...
    vec3 baseColor = color[gl_InstanceID].xyz;
